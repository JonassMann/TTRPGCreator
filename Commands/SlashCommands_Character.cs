...
namespace TTRPGCreator.Commands
{
    public class SlashCommands_Character : ApplicationCommandModule
    {
        ...
        public class CharacterGroup : ApplicationCommandModule
        {
            ...
            public async Task Item(InteractionContext ctx, [Option("character", "Character id")] long character, [Option("item", "Item id")] long item, [Option("quantity", "The number of the item to add. Negative if remove")] double quantity = 1, [Option("equipped", "If the item is equipped by the character")] NullBool equippedBool = NullBool.None)
            {
                await ctx.DeferAsync();
                ...
                var DBEngine = new DBEngine();
                bool querySuccess = await DBEngine.AddCharacterItem(ctx.Guild.Id, character, item, (int)quantity, equipped);
            ...
            public async Task Status(InteractionContext ctx, [Option("character", "Character id")] long character, [Option("status", "Status id")] long status, [Option("level", "The level of the status")] double level = 1)
            {
                await ctx.DeferAsync();

                var DBEngine = new DBEngine();
                bool querySuccess = await DBEngine.AddCharacterStatus(ctx.Guild.Id, character, status, (int)level);
